...
import org.apache.spark.sql.SparkSession;
	...
	public class CMParse
	{
		public static void main(String[] args)
		{
			...
			df.createOrReplaceTempView("test_list");
	        System.out.println("Displays all list for all managed objects");
	        df.show(10, false);
	            
	        //To read all 'list' for a managed object with id=1209282
		    Dataset<Row> result_list = sql.sql("select list from test_list where _id=1401732");
			result_list.show();
			result_list.selectExpr("explode(list) as e").select("e.*").show(100, false);
